    ...
    if await is_heroku():
        if HAPP is None:
            return await message.reply_text(
                "Make sure your HEROKU_API_KEY and HEROKU_APP_NAME are configured correctly in the Heroku config vars"
            )
        heroku_config = HAPP.config()
        if check_var in heroku_config:
            ...
            return await message.reply_text(f"Could not find var {check_var}")
    else:
        ...
        if not output:
            await message.reply_text(f"Could not find var {check_var}")
        else:
            return await message.reply_text(
    ...
    if await is_heroku():
        ...
        if to_set in heroku_config:
            ...
            await message.reply_text(
                f"Successfully added var {to_set} with value {value}"
            )
